...
async function generateSite() {
  ...
  const repoData: RepoData = {
    ...
    codeStructure: {
      ...
      directories: [...new Set(repoFiles.map(f => path.dirname(f.path)).filter(d => d !== '.'))],
      hasTests: true,
      testFiles: repoFiles.filter(f => f.path.includes('test')).map(f => f.path),
      hasTypeScript: true,
      hasTsConfig: true,
      hasPackageJson: true,
...
async function readRepository(repoPath: string): Promise<{path: string, content: string}[]> {
  const readDirRecursive = async (dir: string, base = ''): Promise<{path: string, content: string}[]> => {
    ...
    const files = await Promise.all(
      entries.map(async (entry) => {
        ...
        if (entry.isDirectory()) {
          if (entry.name === '.git' || entry.name === 'node_modules' || entry.name === 'dist' || entry.name === 'docs/sites') {
            return [];
          }
